...
const scriptPath = fileURLToPath(new URL("./packagist-sync.mjs", import.meta.url));
...
describe("Packagist update response", () => {
  test("accepts the documented asynchronous success response", () => {
    expect(parseUpdateResponse(202, { status: "success", jobs: ["job-id"] })).toEqual(["job-id"]);
  });

  test("rejects HTTP and body-level failures", () => {
    expect(() =>
      parseUpdateResponse(403, { status: "error", message: "invalid credentials" }),
    ).toThrow("HTTP 403");
    expect(() => parseUpdateResponse(202, { status: "success", jobs: [] })).toThrow(
      "invalid update response",
    );
    expect(() => parseUpdateResponse(202, { status: "success", jobs: [null] })).toThrow(
      "invalid update response",
    );
  });
  ...
  test("sends the token only in the Bearer header and applies an AbortSignal timeout", async () => {
    ...
    const jobs = await requestPackagistUpdate(
      { token },
      {
        fetchImpl: async (url, options) => {
          ...
          return jsonResponse(202, { status: "success", jobs: ["job-id"], type: "manual" });
        },
    ...
    expect(capturedOptions.headers.Authorization).toBe(`Bearer frndchagas:${token}`);
    expect(capturedOptions.body).toBe(
      JSON.stringify({
        repository: "https://github.com/frndchagas/vinext-laravel-starter-distribution",
      }),
    );
...
describe("Packagist P2 verification", () => {
  test("distinguishes pending, published and mismatched versions", () => {
    ...
    expect(
      inspectP2(p2(publishedVersion(tag, "0000000000000000000000000000000000000000")), {
        tag,
        reference,
      }),
    ).toEqual({
...
describe("Packagist synchronization CLI", () => {
  ...
  test("runs end to end with injected dependencies and prints no token", async () => {
    ...
    expect(output).toEqual([`Packagist published ${tag} from distribution commit ${reference}.`]);
    expect(output.join("\n")).not.toContain(token);
  });
